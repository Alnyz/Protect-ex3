...
import json, ntpath, timeago, time, requests, asyncio, traceback, sys
def loggedIn(func):
...
class Talk(object):
    ...
    def sendMessageWithMention2(self, to, text, mid, text2):
        ...
        if in_text.find(name):
            try:
                ...
                msg.text = text
                msg.contentType = 0
                msg.contentMetadata = {'MENTION':str('{"MENTIONEES":'+json.dumps(arr).replace(' ','')+'}')}
                return self.talk.sendMessage(0,msg)
            except Exception as e:
                raise e
    @loggedIn
    def sendMessageWithMention3(self, to, text='', dataMid=[]):
        arr = []
        list_text = ''
        i = 0
        list_text = ''
        for b,l in enumerate(dataMid,1):
            list_text+= str(b)+'. @[list-'+str(i)+']\n'
            i=i+1
        text = text+list_text+'\nTotal: 「'+str(b)+'」 Users'
        i=0
        for l in dataMid:
    ...
    def grouplist(self,to):
        try:
            ...
            for i in list:
                no += 1
                grup += "\n%i. %s [%s]" % (no,self.getGroup(i).name,len(self.getGroup(i).members))
            grup+="\nTotal group {}".format(len(gid))
            return self.sendMessage(to,str(grup))
        except Exception as e:
            raise e
    @loggedIn
